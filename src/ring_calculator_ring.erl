-module(ring_calculator_ring).

-export([solve/3, violations/2, show/2]).
-export([moves/2, update/3, zero_violations/2]).
...
%test(RingSize, Config, NVals) ->
%  io:format("~s\n", [ring:show(ring:solve(RingSize, Config, NVals), NVals)]).
%
%test_update(RingSize, OldConfig, NewConfig, NVals) ->
